      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, padding: '0 12px 8px', maxHeight: 200, overflowY: 'auto' }}>
        ...
        {stickers.map(s => (
          <div key={s.id} role="button" tabIndex={0} aria-label="发送表情" onClick={() => onSend(s.id)} onKeyDown={e => { if (e.target === e.currentTarget && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onSend(s.id); } }}
            style={{ position: 'relative', cursor: 'pointer', aspectRatio: '1 / 1', borderRadius: 'var(--radius-input)', overflow: 'hidden', background: 'var(--bg-search)', border: '1px solid var(--border-color)' }}>
            ...
            <button onClick={(e) => del(e, s.id)} title="删除"
              style={{ position: 'absolute', top: 2, right: 2, width: 16, height: 16, border: 'none', borderRadius: 'var(--radius-input)', background: 'rgba(0,0,0,.5)', color: 'var(--text-inverse)', fontSize: 11, lineHeight: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
              aria-label="删除表情"
            >✕</button>
          </div>
        ))}
