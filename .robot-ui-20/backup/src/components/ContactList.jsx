      ...
      <>
        ...
        <div style={{ padding: '12px 16px' }}>
          <input
            ...
            maxLength={20}
            aria-label="标签名称"
            style={{ width: '100%', padding: '10px 12px', fontSize: 15, border: '1px solid var(--divider)', borderRadius: 'var(--radius-input)', background: 'var(--bg-card)', color: 'var(--text-primary)', boxSizing: 'border-box' }}
          />
          ...
          <button onClick={saveLabel} disabled={saving || !nameInput.trim()}
            style={{ marginTop: 16, width: '100%', padding: '12px', background: 'var(--green)', color: 'var(--text-inverse)', border: 'none', borderRadius: 'var(--radius-input)', fontSize: 15, cursor: 'pointer' }}>
            {saving ? '保存中…' : '保存'}
          </button>
        </div>
      </>
    ...
    <>
      ...
      <div style={{ padding: '12px 16px 4px', display: 'flex', justifyContent: 'flex-end' }}>
        <button onClick={startCreate}
          style={{ padding: '6px 14px', fontSize: 13, background: 'var(--green)', color: 'var(--text-inverse)', border: 'none', borderRadius: 'var(--radius-2xl)', cursor: 'pointer' }}>
          + 新建标签
        </button>
      </div>
      {labels.length === 0 && (
      ...
      {labels.map(label => (
        <div key={label.id} className="wc-contact-item">
          <div style={{ width: 40, height: 40, borderRadius: 'var(--radius-input)', background: label.color || '#6D5AE6', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <button onClick={() => setShowMembers(label.id)} style={{ marginRight: 8, padding: '4px 10px', fontSize: 12, background: 'transparent', border: '1px solid var(--divider)', borderRadius: 'var(--radius-bubble-tip)', color: 'var(--text-secondary)', cursor: 'pointer' }}>成员</button>
          <button onClick={() => startEdit(label)} style={{ marginRight: 4, padding: '4px 10px', fontSize: 12, background: 'transparent', border: '1px solid var(--divider)', borderRadius: 'var(--radius-bubble-tip)', color: 'var(--text-secondary)', cursor: 'pointer' }}>编辑</button>
          <button onClick={() => deleteLabel(label.id)} style={{ padding: '4px 10px', fontSize: 12, background: 'transparent', border: '1px solid var(--divider)', borderRadius: 'var(--radius-bubble-tip)', color: 'var(--color-badge)', cursor: 'pointer' }}>删除</button>
        </div>
      ))}
    </>
  );
