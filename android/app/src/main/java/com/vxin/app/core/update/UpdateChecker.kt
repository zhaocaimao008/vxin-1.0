...
class UpdateChecker @Inject constructor(
    ...
    /**
     * 远程版本清单 URL 列表（优先 CDN，兜底 www.vxinchat.com 源站）。
     * 返回第一个能成功拉取并解析的结果。
     */
    private val CHECK_URLS = listOf(
        "https://www.vxinchat.com/downloads/vxin-android-version.json",
    )
        ...
        for (url in CHECK_URLS) {
            ...
            if (result.isSuccess) {
                ...
                if (!isTrustedDownloadUrl(dto.url)) {
                    Log.w(TAG, "更新清单 url 校验失败(需 https 且 host=www.vxinchat.com)，跳过该源: ${dto.url}")
                    continue
                }
    ...
    /** 下载直链必须 https 且 host 固定为 www.vxinchat.com，防止清单被篡改后指向任意下载源。 */
    private fun isTrustedDownloadUrl(url: String): Boolean = runCatching {
        val uri = java.net.URI(url)
        uri.scheme.equals("https", ignoreCase = true) && uri.host == "www.vxinchat.com"
    }.getOrDefault(false)
