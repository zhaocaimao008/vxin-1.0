...
webpush.setVapidDetails(
  process.env.VAPID_EMAIL || 'mailto:admin@vxin.app',
  process.env.VAPID_PUBLIC_KEY,
  process.env.VAPID_PRIVATE_KEY
);

// ── Firebase Admin (FCM + APNs) 初始化 ──────────────────────────
...
async function pushToUser(userId, payload) {
  ...
  for (const r of results) {
    if (r.status === 'rejected') {
      console.warn('[push] 推送失败:', r.reason?.message || r.reason);
    }
  }
}

// ── 向会话中的离线用户批量推送 ─────────────────────────────────
// members 参数：socket.js 已查好的成员列表，避免重复 SELECT
async function pushNewMessage({ conversationId, senderId, senderName, content, type, timestamp, onlineUserIds, members: cachedMembers }) {
  // 复用调用方已查好的 members，无需再查一次
  const members = cachedMembers || db.prepare(
  ...
  const pushPromises = members
    ...
    .map(uid => {
      ...
      // 使用 LIMIT 99 早停，与 unread-counts 接口保持一致
