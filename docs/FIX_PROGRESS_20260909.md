...
### 批次 20（2026-09-11）— 真实 Bug：接受好友申请后，新好友在通讯录里"凭空消失"（HTTP 缓存导致）

用户要求继续排查其他 bug。选了此前完全没测过的真实交互链路——好友申请接受流程（之前只测过"申请→出现在新的朋友列表"，没测过"点接受之后联系人列表是否真的更新"）。

- 真实起两个独立账号，A 向 B 发真实好友申请（走 REST API，不是预置好友关系），B 用真实浏览器登录、点"新的朋友"、点"接受"
- **首次截图直接发现异常**：接受后右侧"全部联系人"显示 `(0)` / "暂无联系人"——新好友没有出现。这次没有像批次14/15那样第一时间怀疑测试脚本，而是先分三层证据核实，确认是真 bug 不是误判：
  1. 直接查询隔离测试库的 `contacts` 表：A→B、B→A 两条记录都正确写入了（后端 `handleRequest` 事务本身没问题）
  2. 把 `listContacts` 用的那条 SQL 原样对着同一个库、绑 B 的真实 user_id 单独跑一遍：**能正确查出 A**——证明查询逻辑本身也没问题
  3. 那问题出在哪？——`contacts.controller.js` 的 `listContacts` 响应头写了 `Cache-Control: private, max-age=30, stale-while-revalidate=60`。`ContactList.jsx` 里接受好友申请后会立即调 `fetchContacts()` 想刷新列表，但这次请求和"刚打开页面时"那次是同一个 URL，落在 30 秒缓存窗口内——浏览器直接把本地缓存的"接受前"响应吐回去，连服务器都没真的再请求一次。**连手动刷新页面（`page.reload()`）都不能立刻解决**，因为普通 reload 不清 HTTP 缓存，同一 URL 仍命中缓存
- **修复**：`fetchContacts()` 新增 `fresh` 参数，在确认会改变联系人列表内容的 4 个调用点（接受好友申请的直接刷新 + socket `friend_request_accepted` 广播刷新 + 备注变更刷新 + 删除好友刷新）传 `true`，追加时间戳查询参数绕开这单个请求的浏览器缓存；不影响首次挂载等常规加载场景（继续吃缓存，未改动服务端行为，也没牺牲 30 秒缓存本身对首次加载的性能收益）
- **验证（真实网络层证据）**：Playwright 监听真实网络请求确认修复后请求变成 `GET /api/users/contacts?_=<timestamp>`；同一会话内不刷新页面，接受好友申请后新好友**立即正确出现**在联系人列表（DOM 文本核实）
- **已知残留（如实记录）**：如果用户在接受好友申请后 30 秒内又手动刷新整个页面，仍可能命中同一个不带时间戳参数的旧缓存条目短暂显示为空——这是更窄的边界场景（比"同会话内看不到新好友"影响小得多），要彻底解决需要调整这个接口的缓存策略本身（比如缩短 max-age 或换 ETag 校验），判断为超出本次"精准修复已确认问题"的范围，未继续动
