...
### 批次 15（2026-09-10）— 聊天核心交互细节核查（四.UI，含一次误报排查）

用户要求聚焦"四.UI全面优化"，从聊天窗口最高频的交互细节切入：真实点击表情面板、真实长文本触发输入框自动增高、真实右键消息呼出上下文菜单→点"回复"生成引用条、真实上传图片消息、真实 Tab 键盘导航。

- 表情面板：分类切换（表情/表情包）、网格布局正常
- 长文本输入：三行文本触发 textarea 正确自动增高，未截断
- 右键上下文菜单：回复/编辑/复制/转发/收藏/置顶/撤回/删除不留痕迹菜单项齐全；复核代码确认菜单本身有右/下/上三个方向的边界钳制逻辑（`handleContextMenu` 里的 `MENU_WIDTH/MENU_HEIGHT/PADDING` 计算），不会溢出视口
- 回复引用条：点"回复"后输入框上方正确出现"回复 xxx / 被引用内容"条，带取消按钮
- Tab 键盘导航：连续 6 次 Tab 后焦点correctly落在"设置"导航项上，有清晰可见的绿色焦点环
- **一次误报排查**：截图初见图片消息处是一个空白灰色矩形，疑似图片加载失败。没有直接下结论，而是另起一个专项诊断脚本，等待更久并用 `page.evaluate` 读取 `<img>` 真实状态（`complete`/`naturalWidth`/`classList`）+ 直接 `fetch` 该图片 URL 复核——结果 `complete:true, naturalWidth:100, classList:"wc-msg-img loaded"`，直接 fetch 200。定位为 CSS 里本来就有的 `.wc-msg-img:not(.loaded)` 加载骨架动画（真实存在于 index.css，刻意设计），只是截图时机太早（500ms）撞上了加载中的过渡帧，不是产品缺陷

以上均为真实交互产生的真实截图/真实状态核实，未发现新缺陷。

状态：**未复现**（本批次为核查性质，无新增代码修复）

### 批次 16（2026-09-10）— 登录/注册页移动端窄屏核查

375px 视口（iPhone SE，比常用的 390px 更窄，更容易暴露溢出）真实截图登录页 + 注册页：
- 登录页：品牌区/双 Tab（手机登录/v信登录）/输入框/记住用户名/协议勾选/下载客户端区块，窄屏下无溢出无换行错位
- 注册页：手机号/密码/邀请码三个输入框布局正常；手机号输入框里长号码视觉上只显示后半段是 `<input>` 原生的光标跟随滚动行为（值本身完整，不是截断 bug）
- **确认注册流程本身不采集昵称**（手机号+密码+邀请码即可注册，昵称是注册后在个人资料里补填）——不是这次才发现，是核实"填长昵称测试溢出"这个测试前提时确认的既有产品设计，未发现问题

状态：**未复现**（本批次为核查性质，无新增代码修复）

### 批次 17（2026-09-10）— 真实 Bug：消息发送失败后不会自动滚到底部，用户可能看不到 ❗ 重试图标

继续四.UI核查，测消息组件的"发送失败"态时，用坐标级诊断（`boundingBox`/`isVisible`/直接截图三方核对）发现失败态消息气泡渲染在可视区域之外。

- **根因**：`ChatWindow.jsx` 的贴底自动滚动 effect 用一个"尾部签名"（`messages.length + 最后一条消息的 tempId/id`）去重，避免已读/送达回执之类只改中间行元数据的更新重复触发滚动。但消息从`_status:'sending'`→`_status:'error'`（5秒 ack 超时）时，走的是"原地更新同一条消息的 _status 字段"，`length` 和 `tempId` 都不变——签名和刚发送时完全一样，被去重逻辑直接拦掉，不会重新贴底。首次发送时`forceScrollRef.current=true`确实滚到了底部，但那之后如果视频/图片等前面消息的真实高度还没测完（ResizeObserver 异步），或用户在等待窗口有任何滚动，5秒后真正判定失败时不会再补一次滚动——用户可能完全看不到自己的消息发失败了，也看不到可点击重试的❗图标。
- **修复**：在全部 6 处"判定发送失败"的 `_status:'error'` 赋值前都补上 `forceScrollRef.current = true`（`retryMessage` 的两处 ack 超时/失败回调、文本消息发送的两处、名片消息发送的两处），和"首次发送"用的是同一套已有机制，不引入新逻辑。
- **验证（RED→GREEN，真实浏览器坐标级证据）**：隔离环境真实断网 → 真实点击发送按钮 → 用 `page.evaluate` 读最后一条消息气泡的 `boundingBox`。**RED**（未修复）：`y:653`，落在输入框区域之下，被输入框遮住，真实截图确认看不见。**GREEN**（修复后）：`y:529`，回到可视消息区内，截图确认 ❗图标清晰可见。`git stash` 来回切换两次结果完全对应，不是巧合。

状态：**已修复且已验证**

### 批次 18（2026-09-10）— 真实 Bug：桌面窗口缩放时来电铃声引导条压住新会话的消息内容

用户明确要求把"消息组件/媒体组件/转发收藏弹窗/桌面窗口缩放"四项都过一遍。前三项批次17已覆盖，本批次补第四项——桌面窗口缩放/分栏行为。

- 隔离环境真实建会话，desktop 视口从 1440px 依次缩到 1200/1024/900/768/640/500px 七档，每档用 `document.body.scrollWidth` vs `clientWidth` 检测横向溢出：**全部七档均无横向滚动条**，响应式断点本身工作正常
- **发现真实 Bug**（真实截图，非推测）：在较窄档位（500/640px，实测 1024px 桌面档也有同类现象）下，`CallSoundGuide.jsx`（批次11/12 已经动过两次的那条引导条）会直接压在新会话第一条消息气泡上方——批次11/12 用"猜一个够大的 top 偏移量"应对不同页面的顶部结构高度差异，这次用短会话（消息紧贴顶部）实测证明这个思路本身就不成立：不存在一个放之四海而皆准的偏移值
- **修复**：改用与 `ReconnectingBanner`（同目录，已验证跨页面无遮挡问题的既有组件）完全相同的"顶部通栏细条"方案，不再猜偏移量。已知残留：细条仍会短暂盖住页头最左侧头像/联系人名一小部分（直到用户点击关闭，多数一两秒内发生），判断为比"压住消息正文"轻得多的问题（联系人名在侧边栏同屏可见，是冗余信息短暂遮挡，不是信息缺失），未继续投入解决
- **验证**：七档宽度截图逐一核对，触发过 bug 的档位（500/640/1024px）重新截图确认消息气泡不再被压住；全量 e2e 61/61 通过（含 `UI-VIS-05 无横向滚动条`）

状态：**已修复且已验证**

### 批次 19（2026-09-11）— 首次真实双端通话信令交互验证（之前只做过代码核查）

之前批次4/17 对通话只做了代码层核查（状态机/清理逻辑）和单端"发起→通话窗出现"的既有 e2e 用例，从未验证过接听方视角——`call:incoming` 事件、来电 UI、接听/挂断的完整双端交互链路完全没有真实测试过。本批次补齐：真实起两个独立浏览器 context（各自登录 call_a / call_b，均授予 fake 摄像头/麦克风权限），A 发起语音通话，实测：

- B 端真实收到 `call:incoming` socket 事件，来电 UI（红色拒绝/绿色接听按钮）正确弹出
- B 点接听后，A、B 两端**同步显示"已连接"状态**，含同步计时的通话时长（00:02/00:03），静音/挂断按钮均正确渲染
- **一次差点误判的排查**：A 端挂断后，最初 800ms 后检查 `call-modal` 仍是 visible，一度疑似"挂断没生效"。查代码发现 `CallModal.jsx` 里有 `endCallTimeoutRef.current = setTimeout(onClose, 1800)` 的"通话已结束"过渡态设计，不是bug；重新测更长等待时间：700ms 时modal 内容正确显示"通话已结束"过渡文案，2.3s 后 A、B 两端 modal 均已正常关闭——挂断链路实际完全正常，是我自己测试等待时间不够导致的误判，没有把"未验证先假设有bug"的结论写进报告。

结论：双端通话信令+UI的核心链路（来电→接听→连接→计时→挂断→双端关闭）经真实交互验证工作正常，未发现新缺陷。仍然受限于本机环境：真实音视频流的媒体质量/TURN穿透/真机弱网切换无法验证（fake device 只走信令，不产生真实媒体流）。

状态：**未复现**（核查性质，通话信令链路验证通过，无新增代码修复）

## 附：2026-09-10 生产意外自动部署记录

批次16提交后，本机独立运行的另一自主系统（`/root/scripts/jarvis-autonomy.py`，"JARVIS 自主巡检引擎"）在本轮会话未主动 `git push` 的情况下，将本地 main（含批次6-16共18个commit）推送到了 GitHub 真实远程，触发"自动部署V信后端" workflow 并执行成功（2026-09-10 11:56 UTC，`gh run list` 可查）；生产 `vxin-backend` 进程已重启（pm2 显示 86 分钟前创建）。发现后立即停止后续操作并向用户报告，生产健康检查/首页访问在发现时均正常，未发现故障。批次17（本条）是在用户知情后，经用户明确指示"提交push"才推送。
