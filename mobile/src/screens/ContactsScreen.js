  ...
  return (
    <View style={styles.container}>
      <View style={styles.searchBar}>
        <TextInput style={styles.searchInput} placeholder="搜索用户" placeholderTextColor="#8E8E93" value={search} onChangeText={setSearch} onSubmitEditing={doSearch} returnKeyType="search" />
        <TouchableOpacity activeOpacity={0.82} style={styles.searchBtn} onPress={doSearch}><Text style={styles.searchBtnText}>搜索</Text></TouchableOpacity>
      </View>
      <View style={styles.tabs}>
        {[['contacts','联系人'],['requests',`新朋友${requests.length>0?` (${requests.length})`:''}`]].map(([k,l]) => (
          <TouchableOpacity activeOpacity={0.72} key={k} style={[styles.tab, tab===k && styles.tabActive]} onPress={() => setTab(k)}>
      ...
      <FlatList
        ...
        keyExtractor={i => i.id}
        renderItem={({ item }) => (
          <TouchableOpacity activeOpacity={0.72} style={styles.item} onPress={() => tab==='contacts' && startChat(item)}>
            ...
            {tab === 'search' && <TouchableOpacity activeOpacity={0.82} style={styles.addBtn} onPress={() => sendReq(item.id)}><Text style={styles.addBtnText}>添加</Text></TouchableOpacity>}
            {tab === 'requests' && (
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <TouchableOpacity activeOpacity={0.82} style={styles.acceptBtn} onPress={() => handleReq(item.id,'accepted')}><Text style={styles.acceptText}>接受</Text></TouchableOpacity>
                <TouchableOpacity activeOpacity={0.72} style={styles.rejectBtn} onPress={() => handleReq(item.id,'rejected')}><Text style={styles.rejectText}>拒绝</Text></TouchableOpacity>
              </View>
            )}
          </TouchableOpacity>
        )}
...
const styles = StyleSheet.create({
  ...
  searchBar: { flexDirection: 'row', padding: 10, gap: 8, backgroundColor: 'rgba(247,247,247,0.82)', borderBottomWidth: 1, borderBottomColor: 'rgba(210,214,220,0.72)' },
  searchInput: { flex: 1, backgroundColor: 'rgba(255,255,255,0.74)', borderRadius: 10, padding: 9, fontSize: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.92)' },
  searchBtn: { backgroundColor: '#07C160', borderRadius: 10, paddingHorizontal: 14, alignItems: 'center', justifyContent: 'center', shadowColor: '#07C160', shadowOpacity: 0.18, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 2 },
  searchBtnText: { color: '#fff', fontWeight: '600', fontSize: 13 },
  tabs: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: 'rgba(210,214,220,0.72)', backgroundColor: 'rgba(255,255,255,0.72)' },
  ...
  addBtn: { backgroundColor: '#07C160', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6 },
  addBtnText: { color: '#fff', fontSize: 13, fontWeight: '600' },
  acceptBtn: { backgroundColor: '#07C160', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
  acceptText: { color: '#fff', fontSize: 12 },
  rejectBtn: { backgroundColor: 'rgba(255,255,255,0.78)', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: 'rgba(220,224,230,0.9)' },
