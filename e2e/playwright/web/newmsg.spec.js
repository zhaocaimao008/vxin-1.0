...
async function seedHistory(request, seeded) {
  const tag = `history-${Date.now()}`;
  for (let i = 0; i < 20; i++) {
    const response = await request.post(`${seeded.backendUrl}/api/messages/${seeded.convAB}`, {
      headers: { authorization: `Bearer ${seeded.users[0].token}` },
      data: { type: 'text', content: `${tag}-${i}` },
    });
...
test.describe('新消息悬浮提示 NEWMSG', () => {
  test('NEWMSG-01 上翻看历史 → 出现回到底部按钮', async ({ makeCtx, seeded, baseURL, request }) => {
    test.skip(!seeded.convAB, '无会话');
    const A = seeded.users[0];
    const latest = await seedHistory(request, seeded);
    ...
    // All history is committed before scrolling; pending sends must not move the viewport.
    await chatA.expectMessageVisible(latest);

    // 上翻离开底部 → 「回到底部」按钮出现
    await chatA.scrollMessagesUp();   // 滚到顶
    const m = await chatA.scrollMetrics();
    expect(m.scrollHeight).toBeGreaterThan(m.clientHeight + 300);
    await expect.poll(() => chatA.scrollBottomBtnVisible(), { timeout: 5000 }).toBe(true);
  });

  test('NEWMSG-02 看历史时对方发来 → 角标计数，点按回底清零', async ({ makeCtx, seeded, baseURL, request }) => {
    test.skip(!seeded.convAB, '无会话');
    ...
    expect(mA.scrollHeight).toBeGreaterThan(mA.clientHeight + 300);
    await expect.poll(() => chatA.scrollBottomBtnVisible(), { timeout: 5000 }).toBe(true);
