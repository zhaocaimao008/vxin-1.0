    ...
    <div className="wc-emoji-picker" style={{ padding: '6px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '2px 12px 8px' }}>
        ...
        <button
          ...
          disabled={uploading}
          style={{ fontSize: 12, color: '#07C160', background: 'rgba(7,193,96,.1)', borderRadius: 6, padding: '3px 10px', cursor: 'pointer' }}>
          {uploading ? '上传中…' : '＋ 添加'}
        </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, padding: '0 12px 8px', maxHeight: 200, overflowY: 'auto' }}>
        ...
        {stickers.map(s => (
          <div key={s.id} onClick={() => onSend(s.id)}
            ...
            <img loading="lazy" src={mediaUrl(s.url)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            <button onClick={(e) => del(e, s.id)} title="删除"
              style={{ position: 'absolute', top: 2, right: 2, width: 16, height: 16, borderRadius: 8, background: 'rgba(0,0,0,.5)', color: '#fff', fontSize: 11, lineHeight: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
              aria-label="删除表情"
            >✕</button>
          </div>
        ))}
