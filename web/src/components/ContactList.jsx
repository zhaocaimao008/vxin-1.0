...
export default function ContactList({ onStartChat, searchQuery = '', addFriendRequest = 0 }) {
  ...
  useEffect(() => {
    const handler = (event) => {
      const { userId, remark } = event.detail || {};
      if (userId) {
        setContacts(prev => prev.map(c => c.id === userId ? { ...c, remark: remark || '' } : c));
      }
      fetchContacts();
    };
  ...
  // Sync topbar search text into local state (for display in hints)
  useEffect(() => {
    setSearch(searchQuery);
  }, [searchQuery]);

  // Triggered when user presses Enter in topbar or clicks "添加朋友"
  useEffect(() => {
          ...
          <>
            <div style={{ padding: '8px 16px', background: '#F5F5F5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 13, color: '#888' }}>
                {search ? `"${search}" 的搜索结果` : '添加朋友'}
              </span>
              <button style={{ fontSize: 13, color: '#07C160' }} onClick={() => { setTab('contacts'); setSearch(''); setSearchResults([]); }}>返回</button>
            </div>
            {searching && (
              <div style={{ textAlign: 'center', padding: 24, color: '#B2B2B2', fontSize: 13 }}>搜索中...</div>
            )}
            {!searching && searchResults.map(u => (
              <div key={u.id} className="wc-contact-item" onClick={() => setViewProfile(u.id)}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="wc-contact-item-sub">{u.wechat_id ? `v信号: ${u.wechat_id}` : u.phone}</div>
                </div>
                <span style={{ color: '#C7C7CC', fontSize: 18 }}>›</span>
              </div>
            ))}
            {!searching && searchResults.length === 0 && (
              <div style={{ textAlign: 'center', padding: '40px 16px', color: '#B2B2B2', fontSize: 13 }}>
                <div style={{ fontSize: 36, marginBottom: 10 }}>🔍</div>
                {search
                  ? <><div>未找到用户「{search}」</div><div style={{ fontSize: 12, marginTop: 6 }}>可搜索 v信号、手机号或昵称</div></>
                  : <><div>搜索添加新朋友</div><div style={{ fontSize: 12, marginTop: 6 }}>在顶部搜索框输入 v信号、手机号或昵称，按 Enter 搜索</div></>
                }
              </div>
            )}
          </>
