...
function useDraggable(initial) {
  ...
  return { pos, setPos, onPointerDown, onPointerMove, onPointerUp, wasMoved: () => drag.current.moved };
}
...
const IcoSpeaker = ({ on }) => on
  ? <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.5 12A4.5 4.5 0 0016 7.97v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5zm7 6.17L9.83 13H7v-2h2.83L12 8.83v6.34z"/></svg>
  : <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.5 12A4.5 4.5 0 0016 7.97v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5zm7 6.17L9.83 13H7v-2h2.83L12 8.83v6.34zM21 12c0 2.71-1.15 5.15-3 6.84l1.42 1.42A11.9 11.9 0 0023 12c0-3.86-1.83-7.3-4.68-9.53L17 3.88A9.9 9.9 0 0121 12z"/></svg>;
...
export default function CallModal({ socket, user, call, onClose }) {
  ...
  const remoteVideoRef  = useRef(null);  // full-screen remote video
  const miniVideoRef    = useRef(null);  // minimized bubble remote video
  ...
  /* Minimized bubble — starts top-right */
  const bubble = useDraggable({ x: window.innerWidth - 110, y: 80 });

  /* Local video PiP — starts top-right corner of call screen */
  const pip = useDraggable({ x: window.innerWidth - 130, y: 24 });

  // keep remote stream wired to both video elements
  ...
  // 语音：模糊头像背景
  const voiceBg = remoteUser?.avatar
    ? `url(${mediaUrl(remoteUser.avatar)})`
    : null;
  ...
  return (
    ...
    >
      ...
      {isVideo && <>
        ...
        {/* 本地视频 PiP（可拖拽，仅已连接时显示） */}
        {(status === 'connected' || status === 'connecting') && (
          <div
            style={{
              position: 'absolute',
              left: pip.pos.x, top: pip.pos.y,
              width: 100, height: 140,
              zIndex: 5,
              borderRadius: 14,
              overflow: 'hidden',
        ...
        <div style={{
          ...
          {status !== 'incoming' && (
            <div style={{ flex: 1, textAlign: 'center', marginRight: canMinimize ? 36 : 0 }}>
              <div style={{ fontSize: 16, fontWeight: 600, textShadow: '0 1px 4px rgba(0,0,0,.5)' }}>
                {remoteUser?.name}
              </div>
              {status === 'connected' && (
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,.7)', marginTop: 2 }}>{timer}</div>
              )}
              {status !== 'connected' && (
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,.6)', marginTop: 2 }}>
                  {status === 'calling' ? '等待对方接听…' : '连接中…'}
                </div>
              )}
            </div>
          )}
        ...
        {status === 'incoming' && (
          <div style={incomingCenterStyle}>
            ...
            <div style={{ fontSize: 22, fontWeight: 600, marginTop: 16, textShadow: '0 1px 6px rgba(0,0,0,.6)' }}>
              {remoteUser?.name}
            </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 14, marginTop: 10, color: 'rgba(255,255,255,.7)', minHeight: 20 }}>
            {status === 'connected'  ? <span style={{ color: '#07C160' }}>{timer}</span> :
             status === 'incoming'   ? `${isVideo ? '视频' : '语音'}通话` :
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 4, paddingBottom: 52 }}>
          ...
          {inProgress && status !== 'incoming' && (
            <div style={btnRowStyle}>
              <CircleBtn icon={<IcoMute on={muted} />} label={muted ? '取消静音' : '静音'} active={muted} onClick={toggleMute} />
              <CircleBtn icon={<IcoHangup />} label="挂断" color="#FF3B30" size={68} onClick={() => endCall(true)} testid="call-hangup-btn" />
              <CircleBtn icon={<IcoSpeaker on={speakerOn} />} label={speakerOn ? '关扬声器' : '扬声器'} active={speakerOn} onClick={toggleSpeaker} />
            </div>
          )}
        </div>
      </>}
