...
| 端 | 版本真相源文件 | 字段 | 当前版本 |
|----|--------------|------|---------|
| 桌面端（Windows/Mac/Linux） | `desktop-electron/package.json` | `version` | 8.0.13 |
| 桌面端渲染层内嵌 | `desktop-electron/src/package.json` | `version` | 与上一致（8.0.13，本文件自身不参与打包版本号，仅保持不漂移） |
| Web 前端 | `web/package.json` | `version` | 8.0.16 |
